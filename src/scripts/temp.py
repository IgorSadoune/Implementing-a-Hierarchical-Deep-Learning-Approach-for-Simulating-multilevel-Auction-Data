...
def generate_bids_from_prediction(mu, sigma, n_bidders, seed=None):
    if seed is not None:
        if args.cuda:
    ...
    samples = [torch.normal(mu_i.cpu().detach().repeat(n_i), sigma_i.cpu().detach().repeat(n_i)).numpy() for mu_i, sigma_i, n_i in zip(mu, sigma, n_bidders)]
...
if __name__=="__main__":
    ...
    for data, path in zip(
        ...
        one_hot_n_bidders = data[:, start_idx:end_idx]
        n_bidders = np.concatenate([np.where(row == 1)[0] for row in one_hot_n_bidders])
        print(n_bidders.shape)

        # Generates bids
        pred = generate_bids_from_prediction(mu, sigma, n_bidders, seed=args.seed)
        print(pred.shape)
